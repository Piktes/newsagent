    ...
    <div className="dashboard-page">
      <div className="page-header">
        <h1 style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <BookMarked size={24} /> {listInfo?.name || 'Liste'}
        </h1>
        <button className="btn btn-outline" style={{ gap: '0.375rem', display: 'flex', alignItems: 'center' }} onClick={() => navigate('/favorites')}>
          <ChevronLeft size={16} /> Listeler
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <h3>Bu listede haber yok</h3>
          <p>Haberlerin yıldız butonuna basarak bu listeye ekleyebilirsiniz</p>
        </div>
      ) : (
        <>
          <div className="news-list">
            {news.map(item => (
              <NewsCard key={item.id} item={item} onUpdate={fetchItems} />
            ))}
          </div>
